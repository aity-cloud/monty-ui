...
export async function createAgent(tokenId: string) {
  ...
  (await axios.post<any>(`monty-test/agents`, { token: tokenId, pins: [fingerprint] }));
}

export async function getClusterStats(vue: any): Promise<ClusterStats[]> {
  const clustersResponse = (await axios.get<ClusterStatsList>(`monty-api/CortexAdmin/all_user_stats`)).data.items;
