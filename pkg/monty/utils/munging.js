export function formatForTimeseries(data) {
  ...
  keys.forEach((key) => {
    const translatedKey = this.$store.getters['i18n/withFallback'](`monty.chart.labels.${ key }`, { count: 1 }, key);
  ...
  data.forEach((entry) => {
    Object.entries(entry).forEach(([key, value]) => {
      const label = this.$store.getters['i18n/withFallback'](`monty.chart.labels.${ key }`, { count: 1 }, key);
