...
export async function getDeployments(clusterId: string): Promise<Deployment[]> {
  const deployments = (await axios.get<{ items: DeploymentResponse[] }>(`monty-api/ModelTraining/workload_aggregation/${ clusterId }`)).data;
...
export async function getModelStatus(): Promise<ModelStatus> {
  return await (await axios.get <ModelStatus>(`monty-api/ModelTraining/model/status`)).data;
}
...
export async function trainModel(parameters: ModelTrainingParameters[]) {
  await axios.post(`monty-api/ModelTraining/model/train`, { items: parameters });
}

export async function getModelTrainingParameters(): Promise<ModelTrainingParametersList> {
  const response = (await axios.get<ModelTrainingParametersList>(`monty-api/ModelTraining/model/training_parameters`)).data;
...
export async function hasGpu(): Promise<Boolean> {
  const gpuList = (await axios.get<GPUInfoList>(`monty-api/ModelTraining/gpu_info`)).data;
