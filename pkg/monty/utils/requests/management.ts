...
export async function getClusters(vue: any): Promise<Cluster[]> {
  const clustersResponse = (await axios.get<ClustersResponse>(`monty-api/Management/clusters`)).data.items;
  const healthResponses = await Promise.allSettled(clustersResponse.map(clustersResponse => axios.get<HealthResponse>(`monty-api/Management/clusters/${ clustersResponse.id }/health`)));
...
export async function getCluster(id: string, vue: any) {
  const clusterResponse = (await axios.get<ClusterResponse>(`monty-api/Management/clusters/${ id }`)).data;
