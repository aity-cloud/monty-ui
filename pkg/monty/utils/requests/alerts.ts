...
export async function testAlertEndpoint(request: TestAlertEndpointRequest) {
  await axios.post<AlertEndpoint>(`monty-api/AlertEndpoints/test`, request);
}
...
export async function getAlertConditionsWithStatus(vue: any, clusters: Cluster[]) {
  const response = (await axios.get<ListStatusResponse>('monty-api/AlertConditions/list/withStatus')).data;
...
export function deleteAlertCondition(id: string) {
  return axios.delete(`monty-api/AlertConditions/configure`, { data: { id } });
}
