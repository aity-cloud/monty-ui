...
export default {
  ...
  computed: {
    installCommand() {
      ...
      return `helm -n ${ this.namespace } install monty-agent ${ imageMain } ${ prometheus } --set address=${ this.gatewayAddress } --set pin=${ this.pin } --set token=${ this.token } --create-namespace ${ defaultImageRepository }`;
    },
    gatewayUrl() {
